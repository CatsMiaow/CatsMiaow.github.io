...
?> 불특정 다수에게 불쾌감을 주는 단어가 전체 순위에 노출되면 차단당할 수 있습니다.
...
- !강화 `아이템명`
  ...
  - 아이템 강화는 60초에 한 번만 가능합니다.
    - 강화할 때 아이템이 파괴되면 10분 페널티가 적용됩니다.
...
- `!강화 내순위` 내레벨(최고레벨)과 현재서버(총합레벨)의 전체순위를 보여줍니다.
  - 강화에 참여한 인원/서버 수 기준입니다.
- `!강화 내기록` 강화에 누적된 레벨(업다운)과 횟수(파괴 등)를 보여줍니다.
  - 전체 데이터 초기화 시 같이 초기화됩니다.
...
- !주식 파산 `@멘션`
  ...
  - 지원금을 30회 이상 수령한 유저만 사용할 수 있습니다.
...
- !타자 시작 `한글|영어|한글단문|한글장문|영어단문|영어장문:옵션`
  - 지정된 시간 후에 문장을 보여주고 가장 먼저 정확히 입력한 유저가 우승(?)합니다.
  - 맞는 문장이 없으면 지정된 시간 후에 종료됩니다.
  - 단문의 기준은 `140 bytes` 입니다. 나머지는 장문으로 구분됩니다.
- !타자 시합 `한글|영어 ... :옵션`
  - 위와 동일하지만 3등까지 순위를 집계합니다.
...
- !경마 입장 `이름1 이름2 이름3 ...`
  ...
  - 입장 가능한 말의 수는 `2~8`두 이며, 말의 이름은 20자를 넘을 수 없습니다.
- !경마 종료
...
- !배라 시작
  ...
  - `!배라 참여`로 참가합니다.
- !배라 `숫자1 숫자2 ...`
  - 자기 순서에 `1~31` 범위 내의 숫자를 `1~3`개 입력합니다.
  - 숫자는 공백으로 구분하고 넘어온 숫자에 이어서 연속된 숫자만 입력할 수 있습니다.
  - 마지막에 `31`을 입력한 유저가 최종 탈락합니다.
  - 제한 시간 내에 입력하지 않으면 자동 탈락합니다.
- !배라 종료
  - 진행 중인 배스킨 게임을 중단하고 초기화합니다.
  - 중도 오류 등으로 제대로 종료되지 않을 때 사용할 수 있습니다.
...
- !야구 시작 `자릿수:숫자:옵션`
  - 입력한 자릿수로 경기를 시작합니다. 개인 경기로 시작됩니다.
  - 자릿수를 입력하지 않으면 3자리로 설정됩니다.
  - 일정 시간 입력하지 않거나 입력 횟수를 초과하면 경기가 종료됩니다.
- !야구 `숫자`
  - 설정된 자릿수에 맞는 임의의 숫자를 입력합니다.
  - 응답하는 스트라이크(S), 볼(B), 아웃(O)의 수치로 정답을 예측합니다.
- !야구 종료
  - 진행 중인 경기를 중단하고 초기화합니다.
  - 중도 오류 등으로 제대로 종료되지 않을 때 사용할 수 있습니다.
- 자세한 게임 방식은 아래 링크를 참고하세요.
  - <https://namu.wiki/w/%EC%88%AB%EC%9E%90%EC%95%BC%EA%B5%AC#s-2>
